...
while read line || [ -n "$line" ] ; do
    ...
    echo "python runBARseq.py -i fastq/${fastq} -tagdust-dir ${tagdust} -tagdust-opt ${tag_cmd} -o ${path}/output/${name} -f fixedstruct -iupac ${barcode} -e ${edit_dist} -s ${sat} -c ${min_count} -t 8"
...
jobid=$(swarm --sbatch "--mail-type=BEGIN,END" --time 5-00:00:00 -t 8 -g 160 -f swarm.temp)
