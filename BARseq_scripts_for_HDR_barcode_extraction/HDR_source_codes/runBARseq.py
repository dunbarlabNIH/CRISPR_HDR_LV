...
def save_barseq_outputs(npa, bcode_merge, bcode_ids, bcode_len, sample_lab, out_dir, ed_thr, min_count, saturation, sum_counts):
    ...
    sel_out_name = out_dir + "/" + sample_lab + ".barcode.Alt" + str(min_count) + ".sat" + str(saturation) + ".tsv"
...
def main(raw_args = None):
    ...
    if len(input_files) > 1:
        ...
        sel_df = pd.DataFrame()
        for lab in input_labels:
            td_sel_res = args.out_dir + "/" + lab + ".barcode.Alt" + str(args.min_count) + ".sat" + str(args.saturation) + ".tsv"
