...
tagdust='/data/ginal/utils/tagdust-2.33/src'
#read tagdust
...
while read line || [ -n "$line" ] ; do
    ...
    echo "python runBARseq.py -i fastq/${fastq} -tagdust-dir ${tagdust} -tagdust-opt ${tag_cmd} -o ${path}/output/${name} -f fixedstruct -iupac ${barcode} -e ${edit_dist} -s ${sat} -c ${min_count} -t 2"
...
if [ ${activate} == y ] ; then
	...
	source /data/NHLBI_BCB/komudi/miniconda/conda/etc/profile.d/conda.sh && source /data/NHLBI_BCB/komudi/miniconda/conda/etc/profile.d/mamba.sh
	#conda activate base
...
jobid=$(swarm --sbatch "--mail-type=BEGIN,END" --time 2-00:00:00 -t 8 -g 100 -f swarm.temp)
